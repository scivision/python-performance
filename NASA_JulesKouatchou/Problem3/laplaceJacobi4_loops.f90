...
!----------------------------------------------------------------------
      ...
      double precision :: error, tmp, diff
      double precision :: pi
      integer :: count, i
      real :: start, finish
      ...
      do while ( (count < max_sweep) .and. (error > eps))
         ...
         do j=2,n-1
            do i=2,n-1
               tmp = u(i,j)
               u(i,j) = (4.0d0*(u(i-1,j) + u(i+1,j) + u(i,j-1) + u(i,j+1)) + &
                         u(i-1,j-1) + u(i+1,j+1) + u(i+1,j-1) + u(i-1,j+1))/20.0d0
               diff = u(i,j) - tmp
...
!----------------------------------------------------------------------
      ...
      end program laplaceJacobi4_loops
