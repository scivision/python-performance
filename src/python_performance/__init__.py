...
def compiler_info() -> Dict[str, str]:
    ...
    try:
        if cc == "clang":
            cvers = subprocess.check_output([cc, "-dumpversion"], universal_newlines=True).rstrip()
        elif cc == "gcc":
            ret = subprocess.check_output([cc, "--version"], universal_newlines=True).split("\n")
            cvers = ret[0].split()[-1]
        elif cc == "icc":
            ret = subprocess.check_output([cc, "--version"], universal_newlines=True).split("\n")
        ...
        if fc == "flang":
            fvers = subprocess.check_output([fc, "-dumpversion"], universal_newlines=True).rstrip()
        elif fc == "gfortran":
            ret = subprocess.check_output([fc, "--version"], universal_newlines=True).split("\n")
            fvers = ret[0].split()[-1]
        elif fc == "ifort":
            ret = subprocess.check_output([fc, "--version"], universal_newlines=True).split("\n")
...
def run(cmd: List[str], bdir: Path, lang: str = None) -> Tuple[float, str]:
    ...
    try:
        print("-->", lang)
        ret = subprocess.check_output([exe] + cmd[1:], cwd=bdir, universal_newlines=True).split(
            "\n"
        )
        # print(ret)
